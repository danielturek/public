...
sampler_prior_samples <- nimbleFunction(
    ...
    setup = function(model, mvSaved, target, control) {
        ...
        ind <- 0
        ## checks
        if(length(dim(samples)) != 2)   stop('prior_samples sampler \'samples\' control argument must have dimension = 2', call. = FALSE)
        if(!(storage.mode(samples) %in% c('integer', 'double')))   stop('prior_samples sampler \'samples\' control argument must be numeric or integer type', call. = FALSE)
        if(dim(samples)[2] != k)   stop('prior_samples sampler \'samples\' control argument must be an array with number of columns equal to the number of scalar elements in the \'target\'', call. = FALSE)
    },
    run = function() {
        if(randomDraws) {
        ...
        model$values(targetExpanded) <<- samples[ind, 1:k]
